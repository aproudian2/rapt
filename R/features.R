...
f3features <- function(rvals, fvals_new, fvals_old) {
  ...
  out <- c("F_min_diff" = first(min_diff), "F_min_diff_F" = first(min_diff_F))
  return(out)
}
...
#' @param type a character used for naming the output.  "GH" for guest to host, "HG" for host to guest
#' @return features to be used for machine learning prediction of clustering behavior.  The features are
...
#'
#' @export
g3Xfeatures <- function(rvals, gvals_new, gvals_old, type = "GH") {
  diff <- gvals_new - gvals_old
  min_diff <- min(diff)
  max_diff <- max(diff)
  ...
  names <- c("min_diff", "95diff_r", "FWHM", "absmax_diff", "max_diff")
  names <- sapply(names, function(i) {
    paste("GX", type, "_", i, sep = "")
  })
  names(out) <- names
  return(out)
}
...
k3features <- function(rvals_new, tvals_new, toplot, ...) {
  ...
  peak.info$derivsm <- argmax(rvals_new, -1 * peak.info$deriv,
    w = 3,
    span = span
  )
  peak.info$derivsm_neg <- argmax(rvals_new, peak.info$deriv,
    w = 3,
    span = span
  )

  peak.info$dderiv <- finite_deriv(rvals_new, -1 * peak.info$derivsm$y.hat)
  peak.info$dderivsm <- argmax(rvals_new, peak.info$dderiv,
    w = 3,
    span = span
  )
  ...
  peak.info$ddderivsm <- argmax(rvals_new, peak.info$ddderiv,
    w = 3,
    span = span
  )
  ...
  if (toplot == TRUE) {
    ...
    legend(max(rvals_new) * 0.75, max(tvals_new) * 0.9,
  ...
  out <- (list(
    peak.info$y.hat[peak.info$i[1]], # Tm
    peak.info$x[1], # Rm
